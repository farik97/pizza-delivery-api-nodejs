...
handlers.notFound = function(data, callback) {
    callback(404, {'unatuhorised request': 'unathorised request'})
}
...
handlers._users.PUT = (data, callback) => {
    ...
    if (email && token) {
        ...
        if(name || address || password) {
            ...
            callback(404, {'no data': 'no optional data to change'})
        }

    } else {
        callback(400, {'validation error': 'check the fields'})
    }
...
handlers._menu.GET = (data, callback) => {
    ...
    if (token) {
        ...
        callback(400, {'token error': 'no token or unauthorised token'})
    }
}
...
handlers._orders.POST = (data, callback) => {
    ...
    let token = typeof(data.headers.token) == 'string' && data.headers.token.trim().length == 20 ? data.headers.token.trim(): false
    let cartId = typeof(data.headers.id) == 'string' && data.headers.id.trim().length == 10 ? data.headers.id.trim(): false
    let lastFour = typeof(data.payload.cardNum) == 'string' && data.payload.cardNum.trim().length == 4 || data.payload.cardNum.trim().length == 5 ? data.payload.cardNum: false
            ...
            if (!err && token == tokenData.id) {
                _data.read('carts', cartId, (err, cartData)=>{
                    if (!err && email == cartData.recipient) {
                        if (lastFour == "4242") {
                            cartData.cardType = "tok_visa"
                        }
                        if (lastFour == "5556") {
                            cartData.cardType = "tok_visa_debit"
                        }
                        if (lastFour == "4444") {
                            cartData.cardType = "tok_mastercard"
                        }
                        if (lastFour == "8210") {
                            cartData.cardType = "tok_mastercard_debit"
                        }
                        if (lastFour == "5100") {
                            cartData.cardType = "tok_mastercard_prepaid"
                        }
                        if (lastFour == "1117" || lastFour == "9424") {
                            cartData.cardType = "tok_discover"
                        }
                        if (lastFour == "0004") {
                            cartData.cardType = "tok_diners"
                        }
                        if (lastFour == "0505") {
                            cartData.cardType = "tok_jcb"
                        }
                        if (lastFour == "0005") {
                            cartData.cardType = "tok_unionpay"
                        }
                        if (lastFour == "10005" || lastFour == "98431") {
                            cartData.cardType = "tok_amex"
                        }
                        helpers.sendOrderStripe(cartData, (err)=>{
                            if (!err) {
                                ...
                                let order = {
                                    ...
                                    cardDetails: {
                                        fourDigits: lastFour,
                                        payment_method: cartData.cardType
                                    },
                                ...
                                _data.create('orders', orderId, order, (err)=>{
                                    if (!err) {
                                        helpers.sendEmail(order, (err)=>{
                                            if (!err) {
                                                callback(200, {'order success': 'order successfully created'})
                                            } else {
                                                callback(500, {'mail error': `error: ${err}`})
                                            }
                                        })
                                    } else {
                                ...
                                callback(400, {"stripe error": `error: ${err}`})
                            }
                        })
                    } else {
