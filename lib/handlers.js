                ...
                _data.read('carts', cartId, (err, cartData)=>{
                    if (!err && email == cartData.recipient) {
                        ...
                        _data.create('orders', orderId, order, (err)=>{
                            if (!err) {
                                //  send payment to stripe.com
                                //  if return true update the order status to paymentStatus: true send email using mailgun
                                //  if return false update the order status to paymentStatus: false
                                if (paymentStatus == true) {
                                    _data.read('orders', orderId, (err, orderData)=>{
                                        if (!err && orderData.id == orderId) {
